...
# 1. AWS VPC Module (Multi-AZ Subnets & Redundant Multi-AZ NAT Gateways for Prod)
module "vpc" {
  source = "../../modules/vpc"
  ...
  private_app_subnet_cidrs = ["10.100.11.0/24", "10.100.12.0/24", "10.100.13.0/24"]
  private_db_subnet_cidrs  = ["10.100.21.0/24", "10.100.22.0/24", "10.100.23.0/24"]
...
# 4. EKS Module (Stateless Production Compute: Multi-AZ Managed Node Groups)
module "eks" {
  source = "../../modules/eks"
...
# 5. RDS Module (Stateful Multi-AZ Managed PostgreSQL with PostGIS in Private DB Subnets)
module "rds" {
  source = "../../modules/rds"
...
# 6. ElastiCache Module (Stateful Multi-AZ Redis Replication Group with Auto-Failover)
module "elasticache" {
...
# 8. Workloads Module (High-Availability Production Workload Deployment)
module "workloads" {
...
# 10. ACM TLS Certificate Module (Conditional Production DNS Validation)
module "acm" {
  source = "../../modules/acm"
...
# 11. ALB Ingress Module (High Availability Ingress with WebSocket Support)
module "ingress" {
...
# 12. Route 53 DNS Alias Module (Conditional Production DNS Record)
module "route53" {
